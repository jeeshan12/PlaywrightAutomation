import { test, expect } from "src/fixtures/order-fixture";
...
test(
  ...
  async ({
    ...
    const firstItemCost = await cartPage
      .cartItemPrice(ITEM_ONE_INDEX)
      .innerText();
    const secondItemCost = await cartPage
      .cartItemPrice(ITEM_TWO_INDEX)
      .innerText();
